...
def extract_frames(source: Path, count: int, config: Config, dest_dir: Path) -> Path:
    ...
    for index in range(count):
        ...
        run_ffmpeg(
            ["-ss", f"{stamp:.3f}", "-i", str(source), "-frames:v", "1", "-q:v", "3",
...
def tile(frames_dir: Path, output: Path, config: Config, *, cols: int = DEFAULT_COLS,
         cell_width: int = DEFAULT_CELL_WIDTH) -> Path:
    ...
    run_ffmpeg(
        ["-i", str(frames_dir / FRAME_PATTERN),
         "-vf", f"scale={cell_width}:-1,tile={cols}x{rows}:margin=6:padding=6:color=black",
